...
class GetAdminUserBlocks {
    ...
    private function getGuestsCount($days){
        $bookings = Booking::whereDate('arrival_date', date('Y-m-d',time()-(86400*$days)))->get();
    ...
    private function getTotalCheckIn(){
        $bookings = Booking::whereDate('arrival_date', date('Y-m-d',time()))->get();
        return count($bookings);
    }
    private function getTotalCheckOut(){
        $bookings = Booking::join('booking_rooms','bookings.id','=','booking_rooms.booking_id')
