<div class="btn-group">
    ...
    <div class="dropdown-menu" x-placement="bottom-start">
        ...
        @can('edit-module')
            <a href="{{ route('countries.edit', $country->id) }}" class=" dropdown-item"><i
                    class="cil-pencil action-btn"></i>
                {{ __('Edit') }}</a>
        @endcan
        <div class="dropdown-divider"></div>
        @can('delete-module')
            <a href="{{ route('countries.index') }}" class="dropdown-item  text-danger"
                data-toggle="tooltip" data-original-title="{{ __('Delete') }}"
                onclick="delete_record('delete-form-{{ $country->id }}')"><i
                    class="cil-trash action-btn"></i>{{ __('Delete') }}</a>
            {!! Form::open(['method' => 'DELETE', 'route' => ['countries.destroy', $country->id], 'id' => 'delete-form-' . $country->id]) !!}
            {!! Form::close() !!}
        @endcan
    </div>
</div>
