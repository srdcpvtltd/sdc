<div class="btn-group">
    ...
    <div class="dropdown-menu" x-placement="bottom-start">
        ...
        @can('edit-module')
            <a href="{{ route('cities.edit', $city->id) }}" class=" dropdown-item"><i
                    class="cil-pencil action-btn"></i>
                {{ __('Edit') }}</a>
        @endcan
        <div class="dropdown-divider"></div>
        @can('delete-module')
            <a href="{{ route('cities.index') }}" class="dropdown-item  text-danger"
                data-toggle="tooltip" data-original-title="{{ __('Delete') }}"
                onclick="delete_record('delete-form-{{ $city->id }}')"><i
                    class="cil-trash action-btn"></i>{{ __('Delete') }}</a>
            {!! Form::open(['method' => 'DELETE', 'route' => ['cities.destroy', $city->id], 'id' => 'delete-form-' . $city->id]) !!}
            {!! Form::close() !!}
        @endcan
    </div>
</div>
